	...
	$query = mysqli_query($conn,"select * from student LEFT JOIN class ON student.class_id = class.class_id where status = 'Unregistered' ORDER BY student.student_id DESC") or die(mysqli_error());
		...
		{
			$query = mysqli_query($conn,"select * from student LEFT JOIN class ON student.class_id = class.class_id where status = 'Unregistered' AND student.did=$department_id ORDER BY student.student_id DESC") or die(mysqli_error());
		}
