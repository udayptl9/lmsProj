...
<div class='bottom' style='font-size:1em; font-family:Helvetica; position:relative; bottom:5px'></div>
</body></html><pre>
"; }}}
...
<html><title>Table</title>
	<body bgcolor='#E0FFFF'><center>
	<h4>Enter the grades obtained for each semester</h4>
	<form class='mainForm'>";
	$i=0;
	foreach($this->list as $group) {
		...
		if ($sem >= $group['sem'] && $dep == $group['dep']) {
		$i++;				
		echo "<table class='mainTable' border='1' style='margin-top:50px;' cellpadding='4'>
		<td style='border:;'><h4>Semester {$i}</h4></td></tr>
		
		<tr bgcolor='#E0FFFF'><td><b />Subject</td>
		<td><b />Credit</td>
		<td class='gradeResult'><b />Grade</td>
			</tr>
...
//		$this->creditlist = $group['sub'];
		...
		foreach($group['sub'] as $subs=>$credit) {
			echo "<tr class='subRows'><td>{$subs}</td>
				<td class='subcredit'>{$credit}</td>
				<td><input type='text' name='{$subs}' class='subgrade' maxlength='1'>";}
//$seri = serialize($this->creditlist);
echo "</td></tr>";
}}
$seri = serialize($this->creditlist);
